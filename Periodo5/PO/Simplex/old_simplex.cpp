...
/*Checagem se existe valor negativo no B do auxiliar retorna -1 caso não tenha e, caso tenha, retorna a linha do valor negativo*/
int ChecagemDoBAuxiliar(){
    for(int i = 1; i<(m+1);i++){
...
bool SimplexAuxiliar(){
    ...
    int b_negativo = ChecagemDoBAuxiliar();

    /*Loop enquanto tiver coeficientes negativos na função objetivo e valores negativos na coluna b*/
    while((c_negativo != -1) | (b_negativo != -1)){

        /*Se algum c for negativo temos que arrumar*/
        if(c_negativo != -1 ){
        ...
        /*Se algum b for negativo temos que arrumar*/
        else{if(b_negativo != -1){
            double minimo = inf;
            int coluna_minima = -1;
            double valor;
            double conferir_epsilon;
            ...
            for (int j = 0; j < n+m+m+1; j++){
                if(tableu_auxiliar[b_negativo][j]!= 0.0){tableu_auxiliar[b_negativo][j] = -tableu_auxiliar[b_negativo][j];}
            }

            /*Acha qual dos elementos da linha do b negativo que vai ser o novo pivo*/
            for(int j = m; j<n+m+m;j++){
            
                if(tableu_auxiliar[b_negativo][j]>0){

                    valor = tableu_auxiliar[0][j]/tableu_auxiliar[b_negativo][j];
                    if (valor < minimo){
                        minimo  = valor;
                        coluna_minima = j;
                    }
            ...
            /*Coloca o novo pivo da linha i na vaga i do vetor de pivos*/
            pivos[b_negativo] = coluna_minima;

            /*Fazemos o processo de pivoteamento, o valor nesse caso é o pivo*/
            valor = tableu_auxiliar[b_negativo][coluna_minima];

            /*Primeiro atualizamos a sua linha*/
            for(int j = 0;(j<m+m+n+1);j++){
                conferir_epsilon = tableu_auxiliar[b_negativo][j]/valor;
                if(abs(conferir_epsilon)<epsilon){
                    tableu_auxiliar[b_negativo][j] = 0;
                }
                else{
                    tableu_auxiliar[b_negativo][j] = conferir_epsilon;
                }
            }

            /*Depois zeramos os elementos dessa coluna nas outras linhas, incluso o c*/
            for(int i = 0;(i<m+1);i++){
                if(i != b_negativo){
                    valor = tableu_auxiliar[i][coluna_minima];
                    for(int j = 0;(j<m+n+m+1);j++){
                        conferir_epsilon = tableu_auxiliar[i][j] - tableu_auxiliar[b_negativo][j]*valor;
                        if(abs(conferir_epsilon)<epsilon){
                            tableu_auxiliar[i][j] = 0;
                        }
                        else{
        ...
        b_negativo = ChecagemDoBAuxiliar();
...
bool Simplex(){
    ...
    int b_negativo = ChecagemDoB();

    /*Loop enquanto tiver coeficientes negativos na função objetivo e valores negativos na coluna b*/
    while((c_negativo != -1) | (b_negativo != -1)){


        int coluna_negativa = 0;
        /*Conferimos se tem coluna negativa*/
        int limitado = 1;
        for(int j=m; j<m+n;j++){
            limitado = 0;
            for(int i=1; i<m+1;i++){ 
                ...
                if(tableu[i][m+n]<0){
                    for (int j = 0; j < n+m+1; j++){
                        if(tableu[i][j]!= 0){tableu[i][j] = -tableu[i][j];}
                    }
                }
        ...
        if((!limitado) & (tableu[0][coluna_negativa]<0)){
            CertificadoDeProblemaIlimitado(coluna_negativa);
            Solucao();
            return false;
        }
        ...
        /*Se algum b for negativo temos que arrumar*/
            ...
            /*Invertendo os valores de A*/
            for (int j = 0; j < n+m+1; j++){
                if(tableu[b_negativo][j]!= 0.0){tableu[b_negativo][j] = -tableu[b_negativo][j];}
            }

            /*Acha qual dos elementos da linha do b negativo que vai ser o novo pivo*/
            for(int j = m; j<n+m;j++){
            
                if(tableu[b_negativo][j]>0){

                    valor = tableu[0][j]/tableu[b_negativo][j];
                    if (valor < minimo){
                        minimo  = valor;
                        coluna_minima = j;
                    }
            ...
            /*Fazemos o processo de pivoteamento, o valor nesse caso é o pivo*/
            valor = tableu[b_negativo][coluna_minima];

            /*Primeiro atualizamos a sua linha*/
            for(int j = 0;(j<m+n+1);j++){
                conferir_epsilon = tableu[b_negativo][j]/valor;
                if(abs(conferir_epsilon)<epsilon){
                    tableu[b_negativo][j] = 0;
                }
                else{
                    tableu[b_negativo][j] = conferir_epsilon;
                }
            }
            ...
            for(int i = 0;(i<m+1);i++){
                if(i != b_negativo){
                    valor = tableu[i][coluna_minima];
                    for(int j = 0;(j<m+n+1);j++){
                        conferir_epsilon = tableu[i][j] - tableu[b_negativo][j]*valor;
                        if(abs(conferir_epsilon)<epsilon){
                            tableu[i][j] = 0;
                        }
                        else{
