...
int main()
{
    //fopen
    srand(time(NULL));
    int aux,confirmador,ajuda = 1;

    int linha_inicial,coluna_inicial;
    int casas_visitadas,retrocedidas;

    for(linha_inicial = 0;linha_inicial<8;linha_inicial++)
    {
        for(coluna_inicial = 0;coluna_inicial<8;coluna_inicial++)
        {
            preencher_matriz();
            aux = 1;
            casas_visitadas = 0;
            ...
            printf("Passeio %d para (%i,%i)\n\n",ajuda,linha_inicial,coluna_inicial);
            print_matriz();
