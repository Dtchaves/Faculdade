...
std::string Japonesa::descricao() const {
  ...
  return to_string(m_qtd) + "X Comida japonesa - " + combinado + ", " + to_string(sushis) + " sushis, " + to_string(temakis) + " temakis e " + to_string(hots) + " hots.";
}
...
Japonesa::Japonesa(const std::string& combinado,
                   ...
                   int hots,
                   int qtd,
                   float valor_unitario): combinado(combinado),sushis(sushis),temakis(temakis),hots(hots) {
  m_qtd =qtd;
