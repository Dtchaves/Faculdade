...
bool Expressao_Valida(string Tipo_da_Expressao,string Expressao){
    ...
    if(Conta_Tamanho_Entrada(Expressao)>1000)return false;
    ...
    /*Análise para a expressão infixa*/
        ...
        if(Tipo_da_Expressao == "INFIXA"){
            ...
            //cout<<"Não existe uma expressão do tipo \'"<<Tipo_da_Expressao<<"\'"<<endl;
            return false;
        }
    }
}
...
bool Ler_Expressao(int i,string Entrada,Arvore* Armazenamento){
    ...
    if(Expressao_Valida(Tipo_da_Expressao,Expressao)){
        if(Armazenar(Armazenamento,Expressao,Tipo_da_Expressao)){
            //cout<<"Expressao \""<<Expressao<< "\" lida com sucesso"<<endl;
            return true;
        }
        else{
            //cout<<"A expressao \""<<Expressao<< "\" é inválida"<<endl;
            return false;
        }
    }
    else{
...
void Retornar_Posfixa(bool Expressao_Valida,Arvore* Armazenamento){
    if(Expressao_Valida){
        if(!(Armazenamento->Arvore_Vazia())){
            ...
            cout<<"Ainda não foi fornecido nenhuma expressão para ser armazenada"<<endl;
        }
    }
    else{
        cout<<"Ultima expressão fornecida era inválida, logo não é possível retornar a expressão posfixa"<<endl;
        }
}
...
void Retornar_Infixa(bool Expressao_Valida,Arvore* Armazenamento){
    if(Expressao_Valida){
        ...
        cout<<"Ultima expressão fornecida era inválida, logo não é possível retornar a expressão infixa"<<endl;
        }
}
...
void Retornar_Resultado(bool Expressao_Valida,Arvore* Armazenamento){
    if(Expressao_Valida){
        if(!(Armazenamento->Arvore_Vazia())){
            //cout<<setprecision(6)<<"Resultado = "<<Armazenamento->Retorna_Resultado()<<endl;
        }
        else{
            //cout<<"Ainda não foi fornecido nenhuma expressão para ser armazenada"<<endl;
        }
        ...
        //cout<<"Ultima expressão fornecida era inválida, logo não é possível retornar o resultado"<<endl;
        }
}