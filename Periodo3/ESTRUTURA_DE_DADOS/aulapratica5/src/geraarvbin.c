...
void Pre_(TipoNo *p,TipoChave* Vetor){
    static int Aux_Pre = 0;
    if(p == NULL){
        return;
    }
    else{
    Vetor[Aux_Pre] = p->Reg.Chave;
    Aux_Pre++;
    Pre_(p->Esq, Vetor);
    Pre_(p->Dir, Vetor);
    return;
...
void Pos_(TipoNo *p,TipoChave* Vetor){
    static int Aux_Pos = 0;
    if(p == NULL){
        return;
    }
    else{
    Pos_(p->Esq, Vetor);
    Pos_(p->Dir, Vetor);
    Vetor[Aux_Pos] = p->Reg.Chave;
    Aux_Pos++;
    return;
    }
}

int ancestral(TipoChave i, TipoChave j,TipoChave *Pre_Vetor,TipoChave *Pos_Vetor,int size){

  int Auxiliar = 0;
  int Passador = 0;
  while (((Pre_Vetor[Passador]) != j)&&(Passador <size))
  {
    if(Pre_Vetor[Passador] == i) Auxiliar = 1;
    Passador++;
  }
  if(Auxiliar){
    Auxiliar = 0;
    Passador = 0;
    while ((Pos_Vetor[Passador]) != j)
    {
      Passador++;
    }
    while (Passador < size)
    {
      if(Pos_Vetor[Passador] == i) Auxiliar = 1;
      Passador++;
    }
  }
return Auxiliar;
...
int main(int argc, char *argv[])
{
  ...
  TipoChave Pos_Vetor[size];
    Pos_(root,Pos_Vetor);
  TipoChave Pre_Vetor[size];
    Pre_(root,Pre_Vetor);
  ...
  for (int k=0; k<prm.treesize; k++){
    ...
    if(ancestral(i,j,Pre_Vetor,Pos_Vetor,size)){
