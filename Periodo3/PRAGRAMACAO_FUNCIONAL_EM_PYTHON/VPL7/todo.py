...
def numSummerGoldCountry(games, country):
    ...
    # TODO
    return (games[games['Country'] == country]['GoldS'].sum() + games[games['Country'] == country]['GoldW'].sum())
...
def getNthBestSummerCountry(games, n):
    ...
    # TODO
    return (games.sort_values(by=['GoldS', 'SilverS', 'BronzeS', 'Country'], ascending=[False, False, False, True])).loc[n-1,'Country']
# Test 6
def numCountriesWithMoreThanNWinterMedals(games, n):
    ...
    winter = games['GoldW'] + games['SilverW'] + games['BronzeW']
    return (winter[winter > n]).index.size
...
def numWinterCountries(games):
    ...
    winter = games['GoldW'] + games['SilverW'] + games['BronzeW']
    media = winter.sum()/winter.index.size
    return (winter[winter > media]).index.size - 1
...
def countGoldsWithLetter(games, c):
    ...
    # TODO
    return games[games['Country'][0] == c]['GoldT'].sum()
