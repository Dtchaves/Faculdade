...
def firstDay(profs, prof):
    ...
    """
    aux = list(filter(lambda x: x[2] == prof,buildTurns(profs)))
    return aux[0][0] if len(aux) else "Inexistente"
...
def countTurns(profs, prof):
    ...
    zero.
    """
    return int(reduce(lambda acc,x: acc + 1,filter(lambda y: y[2] == prof,buildTurns(profs)),0))
...
def payTurns(profs, prof):
    ...
    """
    return int(reduce(lambda acc,x: acc + 1000 if x[1] == 'D' else acc + 1333,filter(lambda y: y[2] == prof,buildTurns(profs)),0))
    return 0