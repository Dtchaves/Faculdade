...
def percent(df1, df2, alpha):
  ...
  for i in range (10000):
    ...
    diff[i] = sample1.median() - sample2.median()
    
  IC1 = np.percentile(diff, ((alpha*100/2 )))
  IC2 = np.percentile(diff, (100-((alpha*100/2))))
...
def t_obs(data):
  medias = data.groupby('TEAM').mean()
  return float(medias.loc['Phoenix Suns'] - medias.loc['Charlotte Hornets'])
...
def resposta(data):
  ...
  data_adaptada = data [['TEAM','SALARY']]
  N=10000
  values = np.zeros(N)
  for i in range(N):
    sample = data_adaptada.copy()
    sample['TEAM'] = sample['TEAM'].sample(frac=1)
    mean = t_obs(sample)
    values[i] = mean
  
  IC1 = np.percentile(values,5)
  IC2 = np.percentile(values,95)
  data_mean = t_obs(data_adaptada)
  
  if data_mean > IC1 or data_mean < IC2:
    return False
  else:
    return True
...
def t_obs2(data):
  ...
  data_adaptada = data[data['team_abbreviation'].isin(['CLE', 'GSW'])][['team_abbreviation','player_height']]
  medias = data_adaptada.groupby('team_abbreviation').mean()
  return float(medias.loc['CLE'] - medias.loc['GSW'])
