...
def prob(x, mu, sigma, n):
  ...
  return ss.distributions.norm.sf(((x - mu)/(sigma/np.sqrt(n)))).round(4)
...
def bootstrap_mean(x):
  ...
  for i in range(n):
    ...
    sam = np.random.choice(x,len(x),replace=True)
    means[i] = sam.mean()
    
    
  LI = np.percentile(means,2.5)
  LS =  np.percentile(means,97.5)
...
def t_obs(data):
  ...
  oneF = data[['floors','price']].query("floors == 1")
  notoneF = data[['floors','price']].query("floors != 1")
  return float(oneF['price'].mean() - notoneF['price'].mean())
...
def shuffling(data):
  ...
  IC1 = np.percentile(diffs,2.5)
  IC2 = np.percentile(diffs,97.5)  
  return(IC1,IC2)
...
def resposta():
  ...
  tobs = t_obs(data)
  (IC1, IC2) = shuffling(data)
  if tobs < IC1 or tobs > IC2:
    return True
  else:
    return False
