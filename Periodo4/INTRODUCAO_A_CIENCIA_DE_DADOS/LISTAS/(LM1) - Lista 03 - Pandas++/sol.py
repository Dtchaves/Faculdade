...
def count_missing(df) -> int:
    return df.isnull().sum().sum(axis = 0)
...
def delay(df) -> tuple:
    df_aux = df.dropna(inplace=False)    
...
def high_delay(df):
    df_aux = df.dropna().query('Cancelled == 0').copy()
    idx_same_day = df_aux.CRSArrTime >= df_aux.CRSDepTime  
    df_aux.loc[idx_same_day, 'Q5'] = df_aux.CRSArrTime - df_aux.CRSDepTime          
    df_aux.loc[~idx_same_day, 'Q5'] = 2400 - df_aux.CRSDepTime + df_aux.CRSArrTime  
    df_aux = df_aux[['OriginAirportName', 'DestAirportName', 'Q5']]    
    df_aux = df_aux.groupby(['OriginAirportName', 'DestAirportName']).mean()
    max_values = df_aux['Q5'].idxmax()
    return max_values
...
df_aux = df.dropna().query('Cancelled == 0').copy()
plt.scatter(df_aux['DepDelay'],df_aux['ArrDelay'], edgecolor='k', alpha=0.75)
plt.xlabel('DepDelay')
plt.ylabel('ArrDelay')
