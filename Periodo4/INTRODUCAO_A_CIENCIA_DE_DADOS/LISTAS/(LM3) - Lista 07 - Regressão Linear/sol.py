...
def mean_error(alpha, beta, x, y):
  # SEU CODIGO AQUI
  return y.mean() - (x.mean()*beta + alpha) 
...
y_reta = (x*beta + alpha)
plt.scatter(x, y, alpha=0.8, edgecolors='k', s=80)
plt.plot(x, y_reta, color='magenta')
plt.xlabel('Altura média dos pais')
plt.ylabel('Altura dos filhos')
plt.title('Gráfico de dispersão e a reta gerada pelo modelo')
despine()
...
assert_equal(np.round(erro_medio2, 2), 10.14)
...
def sum_of_squared_errors(alpha, beta, x, y):
  # SEU CODIGO AQUI
  return np.pow((y - (x*beta + alpha)),2).sum()
...
def least_squares_fit(x, y):
  # SEU CODIGO AQUI
  covxy =   pearsonr(x,y) [0]
  beta = covxy * np.std(y,ddof = 1)/np.std(x,ddof = 1)
...
def r_squared(alpha, beta, x, y):
  # SEU CODIGO AQUI
  value1 = ((y - (x*beta + alpha))**2).sum()
  value2 = ((y -y.mean())**2).sum()
